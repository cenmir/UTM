...
class SerialManager(QObject):
    ...
    load_cell_data = pyqtSignal(float, int)  # Parsed load cell value, MCU timestamp (ms)
    ...
    def _parse_response(self, line):
        ...
        try:
            ...
            if line.startswith("Welcome to"):
                ...
                # Try to parse as load cell data
                # New format: "force,mcu_timestamp_ms"
                # Old format: "force" (backward compatibility)
                try:
                    if ',' in line:
                        parts = line.split(',')
                        value = float(parts[0])
                        mcu_ts = int(parts[1])
                        self.load_cell_data.emit(value, mcu_ts)
                    else:
                        value = float(line)
                        self.load_cell_data.emit(value, 0)  # 0 = no MCU timestamp
                except ValueError:
