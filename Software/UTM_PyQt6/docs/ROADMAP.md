...
>   `documentation/decks/E_modulus_explained.pptx` slide 5 (the original S16 observation).
>
...
- ✅ **POSTERS — BUILT 2026-08-12, THREE EDITIONS from one content spec.**
  `documentation/scripts/generate_poster.py` → **`documentation/posters/`** (pptx + pdf for each):
  ...
  - **Rig photographs are background-removed cutouts** (`documentation/scripts/rig_cutouts.py`), which is why
  ...
  - New assets: `documentation/figures/feat_prepare_specimen.png` + `feat_release_load.png` (SF2/SF8 had no proof
    figure at all — generated by `documentation/scripts/poster_assets.py`); `rig_photo.jpg`, `rig_photo_detail.jpg`,
...
- 🟡 **DECK — bring `documentation/decks/V6a_8_6_20_slides.pptx` up to current progress.**
  ✅ **SF9 built 2026-08-10 — 15 slides, pages 186-200, deck now 60 slides:** overview of all six modes (real measured signatures, not schematics) · 2 slides per mode (how it works + settings + **software limits**, then results with graphs and tables) · **T7 failure analysis** (why it stalled, ranked causes, mitigations). Numbers are computed by `documentation/scripts/sf9_data.py` from the rig CSVs and imported by the generator, so nothing on these slides is transcribed by hand.
...
- ✅ **NOT A TORQUE CEILING AT ALL — RESOLVED 2026-08-12. It was crosshead misalignment.** The load
  ...
  binding instead of on the specimen. Re-aligned and re-tightened, the rig pulls **3.5 kN with no
  stutter**. Deck **p183** (root cause) / **p184** (post-fix register); `documentation/scripts/torque_fix_data.py`
