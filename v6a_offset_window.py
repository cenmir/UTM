"""V6a single-common-offset-factor analysis (same interval-intersection method as V5).
...
M = {"E": 2.41, "σ_y": 47.80, "UTS": 47.82}          # V6a measured (GPa / MPa)
...
# conflict: E floor vs sigma_y ceiling
ax.annotate("", xy=(win["E"][0], 3.45), xytext=(win["σ_y"][1], 3.45),
            arrowprops=dict(arrowstyle="<->", color="#b00020", lw=2.0))
ax.text((win["E"][0]+win["σ_y"][1])/2, 3.6,
        f"gap {win['E'][0]-win['σ_y'][1]:.2f} → NO all-property k",
        ha="center", color="#b00020", fontweight="bold", fontsize=11.5)
ax.set_yticks([]); ax.set_xlabel("offset factor  k = literature / measured")
ax.set_xlim(0.45, 2.45); ax.set_ylim(0.4, 3.95)
ax.set_title("V6a (100 % infill) — feasible offset-factor window per property", fontweight="bold")
ax.legend(loc="lower right", fontsize=11); ax.grid(axis="x", alpha=0.3)
plt.tight_layout(); plt.savefig("V6a_offset_window.png", dpi=150, bbox_inches="tight"); plt.close()
